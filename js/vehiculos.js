...
function editCar(obj){
  ...
  $("#enlacecedit").val($("#editEnlace"+idECar).html());
  ...
  $.getJSON( host+request, function( data ) {
    ...
    if(data.length>0){
      for(var i=0;i<data.length;i++){
        ...
        contentCar += '<div class="collapsible-body"><a class="waves-effect waves-light btn gflores btnEditCar" style="float:right;margin:10px;" item-id="'+data[i].id+'" onclick="editCar(this)">Editar</a><a class="waves-effect waves-light btn gflores" style="float:right;margin:10px;" item-id="'+data[i].id+'" onclick="goGallery(this)">Galer&iacute;a</a><a class="waves-effect waves-light btn gflores" onclick="confirmDeleteCar(this)" style="float:right;margin:10px;" item-id="'+data[i].id+'">Eliminar</a><img src="images/cars/'+data[i].foto+'" style="width:auto;margin:auto;margin-top:0px;width:50%;margin-left:25%;"/><p>'+data[i].descripcion+'<br><br>PDF: '+data[i].pdf+'<br>Enlace: '+data[i].enlace+'</p></div></li>';
        contentCar += '<div style="display:none;"><span id="editnombre'+data[i].id+'">'+data[i].nombre+'</span><span id="editPdf'+data[i].id+'">'+data[i].pdf+'</span><span id="editDescription'+data[i].id+'">'+data[i].descripcion+'</span><span id="editYear'+data[i].id+'">'+data[i].year+'</span><span id="editEnlace'+data[i].id+'">'+data[i].enlace+'</span></div>';
